...
def decoder(image_array, nl_original, nc_original):
    matrix_joined_rgb = Ex3_4_2(R, G, B)
    ...
    ycbcr = RGB_toYCbCr(fig)
    R, G, B = YCbCr_to_RGB(ycbcr)
...
def main():
    ...
    ycbcr = RGB_toYCbCr(img)
    showCanals(ycbcr) 
    YCbCr_to_RGB(ycbcr)
    
    lines, columns = img.shape
    
    img_padded = encoder(img, color1, color2, colormap)
    
    decoder(img_padded, lines, columns)
